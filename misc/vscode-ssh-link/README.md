    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.claude/scripts/vscode-ssh-link.sh <YOUR_HOSTNAME>",
            "timeout": 5
          }
        ]
      }
...
- Bare path: `RestAPIAccessor/ImportExport/Unzipper.cpp`
- Path-with-line in prose: `Unzipper.cpp:171`
- Any file mention without the `vscode://...` link

**If you are about to send a response, scan it first.** Every path-shaped token must be a VS Code remote link. If even one is bare, fix it before sending. Treat this with the same rigor as the `rg`-vs-`grep` rule above.
...
- `<hostname>` = `devcc`
- `<absolute_file_path>` — full path without leading `/` (e.g. `data/i565578/ws/hana3/RestAPIAccessor/ImportExport/Unzipper.cpp`)
...
- `<label>`: relative path or filename (e.g. `src/main.cpp:42` or `main.cpp:42`)

Examples:

[RestAPIAccessor/ImportExport/Unzipper.cpp:171](vscode://vscode-remote/ssh-remote+devcc/data/i565578/ws/hana3/RestAPIAccessor/ImportExport/Unzipper.cpp:171)
[RestAPIAccessor/ImportExport/Unzipper.cpp](vscode://vscode-remote/ssh-remote+devcc/data/i565578/ws/hana3/RestAPIAccessor/ImportExport/Unzipper.cpp:1)
[Unzipper.cpp](vscode://vscode-remote/ssh-remote+devcc/data/i565578/ws/hana3/RestAPIAccessor/ImportExport/Unzipper.cpp:1)
````

## How It Works

| Scenario | Link generated by |
|----------|-------------------|
| Claude executes `Edit`/`Write`/`NotebookEdit`/`Read`/`Grep` | Hook (automatic) |
| Claude mentions a code location in text | CLAUDE.md instruction (Claude writes it) |
